...
def forward(bbox,tracks, fwd = False): # эта функция позволяет сохранить лист детекций в который внесены айди от трека сохраняя нетрекованные боксы на случай последующей перетрековки
...
def tracking_on_detect(all_boxes,tracker,orig_shp): # эта функция отправляет нетрекованные боксы людей в треккер прокидывая остальные классы мимо треккера
  ...
  for i in range(int(max(all_boxes[:, -1]))):
    ...
    person = forward(bbox,tracks, fwd = False) # эта функция позволяет использовать далее лист детекций в который внесены айди от трека (трек фильтрует и удаляет боксы) 
